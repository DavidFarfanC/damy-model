...
def create_app(engine: InferenceEngine) -> FastAPI:
    ...
    async def predict_endpoint(
        ...
        try:
            ...
            encoded = jsonable_encoder(result, exclude_none=False)
            safe_content = _sanitize_json(encoded)
    ...
    async def predict_batch(request: BatchPredictRequest) -> BatchPredictResponse:
        ...
        encoded_results = jsonable_encoder(results, exclude_none=False)
        safe_results = _sanitize_json(encoded_results)
        return BatchPredictResponse(results=safe_results)
